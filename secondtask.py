def hit(x1,y1,x2,y2):
    ...
    if x1 != x2 and y1 != y2:
        ...
        while l[x1 - up_right][y1 + up_right] != "x":
            if x1 - up_right == x2 and y1 + up_right == y2:
                return True
            
            up_right += 1

            
        while l[x1 - up_left][y1 - up_left] != "x":
            if x1 - up_left == x2 and y1 - up_left == y2:
                return True
            
            up_left += 1
            
        while l[x1 + down_right][y1 + down_right] != "x":
            if x1 + down_right == x2 and y1 + down_right == y2:
                return True

            down_right += 1
            
        while l[x1 + down_left][y1 - down_left] != "x":
            if x1 + down_left == x2 and y1 - down_left == y2:
                return True
